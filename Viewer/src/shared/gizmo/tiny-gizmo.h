...
template <class T>
T minalg::determinant(const mat<T, 4, 4> &a) {
...
struct gizmo_application_state {
  ...
  float screenspace_scale{
      0.f};  // If > 0.f, the gizmos are drawn scale-invariant with a
             // screenspace value defined here
...
struct gizmo_context {
  ...
  std::function<void(const geometry_mesh &r)>
      render;  // Callback to render the gizmo meshes
};
